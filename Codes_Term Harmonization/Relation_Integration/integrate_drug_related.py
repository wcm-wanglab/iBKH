...
@ Author: Yu Hou
@File: integrate_drug_related.py
@Time: 4/25/21 11:57 PM
"""
...
def integrate_DPwy():
    D_Pwy_kegg = pd.read_csv(folder + 'KEGG/kegg_drug_pathway.csv')
    print(D_Pwy_kegg)
    pathway_vocab = pd.read_csv(folder + 'v2_res_Apr2021_refine/res/entity/pathway_vocab.csv')
    drug_vocab = pd.read_csv(folder + 'v2_res_Apr2021_refine/res/entity/drug_vocab.csv')
    ...
    print(D_Pwy_res)
    D_Pwy_res.to_csv(folder + 'v2_res_Apr2021_refine/drug_related/D_Pw_res.csv', index=False)


def integrate_DSE():
    sider_df = pd.read_table(folder + 'stage_4/entity/side_effect/meddra_all_se.tsv', header=None)
    ...
    drug_vocab = pd.read_csv(folder + 'v2_res_Apr2021_refine/res/entity/drug_vocab.csv')
    side_effect_vocab = pd.read_csv(folder + 'v2_res_Apr2021_refine/res/entity/side_effect_vocab.csv')
    ...
    print(D_SE_res)
    D_SE_res.to_csv(folder + 'v2_res_Apr2021_refine/drug_related/D_SE_res.csv', index=False)


def integrate_DSDSI():
    sdsi_spd = pd.read_table(folder + 'stage_3/idisk-rrf-1.0.1-2020-01-28/MRREL.RRF', delimiter='|')
    ...
    sdsi_spd_res.to_csv(folder + 'v2_res_Apr2021_refine/drug_related/SDSI_D_res.csv', index=False)
